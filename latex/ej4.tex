...
\begin{itemize}
    ...
    \item Grafos con distintas componentes conexas conectadas entre sí, consultando por distintos pares $\langle a, b \rangle$ tales que $a$ y $b$ corresponden a la misma componente, tales que $a$ y $b$ pertenecen a distintas componentes y se puede ir de $a$ a $b$ pero no volver, y tales que $a$ y $b$ pertenecen a distintas componentes y se puede volver de $b$ a $a$ pero no ir.
    \item Grafos generados aleatoriamente con varias componentes fuertemente conexas tanto conecatadas entre sí como no, de dimensiones cercanas a las cotas propuestas para el testeo en el enunciado para corroborar el funcionamiento correcto de la implementación ante la mayor carga de datos.
